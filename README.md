...
`Feishu Image To Sheet Cell v0.8.0` 可以把 ComfyUI 的 `IMAGE` 写入普通飞书 Sheet 的单个单元格。

典型连接方式：

```text
BAResourceConvert(output_type=图片) -> Feishu Image To Sheet Cell v0.8.0(image)
```
...
推荐使用 `Feishu Value To Sheet Cell v0.8.0`。它可以按行列写入文本或图片：
...
使用 `Feishu Sheet Cell Reader v0.8.0` 从普通飞书 Sheet 的指定单元格读取内容：
...
图片读取会优先识别单元格返回值中的 `box...` 或 `fileToken` 素材 token，包括飞书把图片对象作为 JSON 字符串返回的情况，并额外查询同一单元格上的浮动图片。若 `auto` 模式读到单元格图片结构但直接下载失败，或 `read_mode=image` 找不到可直接下载的 token，节点会自动把表格导出为 XLSX，并从 Excel 内嵌图片锚点中提取目标单元格图片。这个回退路径会稍慢，并需要应用有电子表格导出/下载相关权限；如果仍找不到图片，节点会返回空图和 `has_image=false`，同时把候选图片锚点写进 `status_json` 方便诊断。
